...
func StringToHexColor(str string) string {
	...
	// Make the string a length that is a multiple of 3 by adding zeros
	for len(str)%3 > 0 {
		str += "0"
	}

	// Separate the string into 3 equal length strings
	n := len(str) / 3
	a, b, c := str[:n], str[n:2*n], str[2*n:]

	// Truncate each string to 2 characters
	a, b, c = strings.ToLower(a[:2]), strings.ToLower(b[:2]), strings.ToLower(c[:2])

	// Keep the hex values, and add zeros where necessary
	str = a + b + c
	final := "#"
	for _, c := range str {
		if isHexadecimalCharacter(string(c)) {
			final += string(c)
		} else {
			final += "0"
		}
...
func IsReserved(str string) (reserved bool, hexadecimalValue string) {
	...
	// Match something like "ff0099"
	if len(str) == 6 && isHexadecimal(str) {
		return true, "#" + strings.ToLower(str)
	}

	// Match something like "red"
	if value, hasKey := getReservedWords()[strings.ToLower(str)]; hasKey {
...
func isHexadecimalCharacter(character string) bool {
	r, _ := regexp.Compile(`^[0-9a-f]{1}$`)
	return r.MatchString(strings.ToLower(character))
}

func isHexadecimal(str string) bool {
	r, _ := regexp.Compile(`^[0-9a-f]{6}$`)

	return r.MatchString(strings.ToLower(str))
}

func getReservedWords() map[string]string {
